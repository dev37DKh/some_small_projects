...
grid_cost_per_kWh = 0.60 # Example grid electricity price
model.Cost = Objective(
    expr=model.C_PV * cost_PV +
         model.C_Wind * cost_Wind +
         model.C_Battery * cost_Battery +
         model.C_Hydrogen * cost_Hydrogen +
         sum(grid_cost_per_kWh * model.Grid_Import[t] for t in model.T),
    sense=minimize
)
...
opt = SolverFactory('cbc')  # Use any LP solver like GLPK or CBC
